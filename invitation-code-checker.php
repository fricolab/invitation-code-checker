...
class Invitation_Code_Checker {
	...
	function add_code_field_to_signup( $errors ) {
		...
		<?php } ?>
		<input name="invitation_code" style="font-size:24px; margin:5px 0px; width:100%;" type="text" id="invitation_code" value="<?php echo esc_html( $_POST['invitation_code'], 1 ) ?>" maxlength="200" /><br /><?php _e( 'The registration is only permitted with a valid invitation code.', 'invitation-code-checker' ) ?>
	<?php }
	...
	function add_code_field_to_signup_buddypress() {
		...
		<div class="register-section" id="invitation-code-section" style="margin:0 0 40px; clear:left; width:48%;">
			...
			<?php } ?>
			<input type="text" name="invitation_code" id="invitation_code" style="width:50%;" value="<?php echo esc_html( $_POST['invitation_code'], 1 ) ?>" />
		</div>
	<?php }
	...
	?>
		<input type="hidden" name="invitation_code" value="<?php echo esc_html( $_POST['invitation_code'], 1 ) ?>" />
	<?php }
}
